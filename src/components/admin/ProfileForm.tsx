  ...
  return (
    <div className="rounded-xl border border-[var(--line)] p-4">
      ...
      <div className="flex items-center gap-3">
        ...
        />
        <span className="ml-1 h-7 w-7 rounded-full border border-[var(--line)]" style={{ background: value || "var(--accent)" }} />
      </div>
      {inheritLabel && (
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <section className="grid gap-5 rounded-2xl border border-[var(--line)] bg-[var(--ink-soft)]/40 p-5 md:grid-cols-[220px_1fr]">
        <ImageUploader name="avatarUrl" folder="avatar" initialUrl={profile.avatarUrl} label="Ảnh đại diện" aspect="aspect-square" maxDim={1200} />
      ...
      {/* ---------- CV sheet ---------- */}
      <section className="space-y-4 rounded-2xl border border-[var(--line)] bg-[var(--ink-soft)]/40 p-5">
        <div>
          <h3 className="text-sm font-semibold">Trang CV (Compact sheet)</h3>
          <p className="mt-1 text-xs text-fg-faint">
            Trang /about luôn mở ở chế độ Compact. Các trường dưới đây điều khiển khối tiêu đề, gáy tên dọc và trạng thái.
          </p>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <Select
            label="Trạng thái (góc STATUS)"
            name="status"
            defaultValue={profile.status}
            options={[
              { value: "open", label: "OPEN TO WORK (xanh)" },
              { value: "busy", label: "BUSY (đỏ)" },
            ]}
          />
          <Select
            label="Ngôn ngữ mặc định khi mở CV"
            name="defaultLang"
            defaultValue={profile.defaultLang}
            options={[
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Mã bản vẽ (SHEET)" name="sheetId" defaultValue={profile.sheetId} placeholder="CV-01" />
          <Field
            label="Revision — để trống sẽ tự lấy tháng lưu gần nhất"
            name="revision"
            defaultValue={profile.revision}
            placeholder="REV 2026.08"
          />
        </div>
        <Field
          label="Gáy dọc — dòng vai trò (màu accent)"
          name="spineRoles"
          defaultValue={profile.spineRoles}
          placeholder="ARCHITECT · BIM COORDINATOR · BIM MANAGER"
        />
        <Field
          label="Gáy dọc — dòng dịch vụ (màu xám)"
          name="spineServices"
          defaultValue={profile.spineServices}
          placeholder="BIM MODELING · BIM CONSULTANT · BIM SOLUTION"
        />
      </section>
      ...
      <section className="space-y-4 rounded-2xl border border-[var(--line)] p-5">
        <div>
          ...
          <p className="mt-1 text-xs text-fg-faint">
            Accent đổi màu nút, link, highlight và toàn bộ thanh/chip trên tờ CV. Màu chữ tiếng Anh áp dụng cho mọi dòng EN.
          </p>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <ColorField name="accent" label="Màu accent (tone chính)" defaultValue={profile.accent} />
          <ColorField name="enColor" label="Màu chữ tiếng Anh" defaultValue={profile.enColor} inheritLabel="Dùng cùng màu accent" />
        </div>
        <Select
      ...
      <section className="rounded-2xl border border-[var(--line)] p-5">
        <h3 className="mb-4 text-sm font-semibold">Tóm tắt chuyên môn (01 / CV)</h3>
        <BiArea label="Tóm tắt" base="summary" vi={profile.summaryVI} en={profile.summaryEN} rows={5} />
      </section>
