...
import { Reveal, Stagger, StaggerItem, MaskReveal } from "@/components/ui/motion";
...
export function SelectedWork({
  ...
  const picked = projects.filter((p) => p.featured).sort((a, b) => a.featuredOrder - b.featuredOrder);
  const featured = picked.length ? picked : projects.slice(0, 4);
        ...
        <div className="mb-12 flex items-end justify-between gap-6">
          <div>
            ...
            <h2 className="mt-3 font-display text-4xl font-light tracking-tight sm:text-5xl">
              <MaskReveal className="vi">Dự án tiêu biểu</MaskReveal>
            </h2>
          </div>
      ...
      <Stagger className="grid gap-6 md:grid-cols-2">
        {featured.map((p, i) => (
          <StaggerItem key={p.id}>
            <ProjectCard project={p} index={i} priority={i < 2} aspect={p.aspect || defaultAspect} />
          </StaggerItem>
        ))}
      </Stagger>
