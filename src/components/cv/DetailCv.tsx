...
/**
 * Detail view = the original site's About layout, unchanged:
 * bilingual (VI + EN shown together), profile card, summary, experience,
 * education, skills. Rendered inside the normal site chrome (header + footer).
 */
...
export function DetailCv({ data }: { data: SiteData }) {
    ...
    <>
      <Reveal>
        <header className="mb-16 max-w-4xl">
          ...
          <p className="mt-3 font-mono text-sm uppercase tracking-[0.25em] text-accent">{profile.role}</p>
        </header>
      </Reveal>
                    ...
                    <ul className="mt-4 space-y-3">
                      {e.achievementsVI.map((a, i) => (
                        <li key={i}>
                          ...
                          <span className="en block text-xs leading-relaxed">{e.achievementsEN[i] ?? ""}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
