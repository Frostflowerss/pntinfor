...
function ProjectModal({ project, onClose }: { project: WorkProject; onClose: () => void }) {
  const galleryImages = useMemo(() => {
    // In project detail, do not show cover assets.
    return (project.images ?? []).filter(
      (src) => src && src !== project.cover && !src.includes("/cover-")
    );
...
export function WorkCarousel() {
  ...
  // When user swipes to the clone edges, jump instantly to the matching real slide.
  useEffect(() => {
    jumpIfClone(activeVirtualIndex);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeVirtualIndex]);
