...
import { Plus, Pencil, ImageOff, Star } from "lucide-react";
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-fg-faint">
            {projects.length} dự án · {projects.filter((p) => p.featured).length} tiêu biểu (mục 01)
          </p>
        </div>
        <Link
        ...
        <div className="space-y-3">
          {projects.map((p) => (
            ...
            >
              ...
              <form action={toggleProjectFeatured.bind(null, p.id, !p.featured)}>
                <button
                  type="submit"
                  title={p.featured ? "Bỏ khỏi mục Tiêu biểu" : "Đưa vào mục Tiêu biểu (01)"}
                  className={`grid h-8 w-8 place-items-center rounded-lg transition hover:bg-white/5 ${
                    p.featured ? "text-accent" : "text-fg-faint hover:text-fg"
                  }`}
                >
