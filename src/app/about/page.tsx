        ...
        <Column className={styles.blockAlign} flex={9} maxWidth={44}>
          ...
          {about.technical.display && (
            <>
              ...
              <SkillGrid
              skills={about.technical.skills.map((s: any) => ({
                title: s.title,
                level: (s.tags?.[0]?.name ?? "").toString(),
                percent: levelToPercent((s.tags?.[0]?.name ?? "").toString()),
                column: (s.column ?? undefined) as 1 | 2 | undefined,
              }))}
            />
            </>
          )}
